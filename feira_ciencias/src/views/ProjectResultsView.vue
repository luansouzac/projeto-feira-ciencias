...
// --- Estado da Página ---
const projeto = ref(null); // Para guardar os detalhes do projeto
...
onMounted(async () => {
  ...
  try {
    // Busca os dados do projeto e as suas avaliações em paralelo
...
// Calcula a nota média geral de todas as avaliações
...
// Agrupa todas as respostas por critério para calcular médias
const notasPorCriterio = computed(() => {
  const criterios = {};
  if (!avaliacoes.value) return [];

  // Junta todas as respostas de todas as avaliações
  ...
  // Calcula a média para cada critério
