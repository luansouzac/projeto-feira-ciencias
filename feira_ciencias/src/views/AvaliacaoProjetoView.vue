...
onMounted(async () => {
  ...
  try {
    ...
    erro.value =
      ...
      error.message ||
      'Não foi possível carregar os dados para a avaliação.'
  } finally {
    loading.value = false
  }
})
    ...
    <v-container class="py-10">
      ...
      <div v-else-if="projeto">
        ...
        <v-card class="mb-8 text-center pa-4" color="green-darken-4" theme="dark">
          ...
          <v-card-title class="text-h4 font-weight-bold text-wrap">{{
            projeto.titulo
          }}</v-card-title>
        </v-card>
        ...
        <v-card v-if="!isAluno" variant="flat" border>
          <v-card-text class="pa-6">
            <p class="text-h6 font-weight-regular text-medium-emphasis mb-6">
              Preencha os campos abaixo de acordo com a sua análise do projeto.
            </p>

            <v-expansion-panels variant="accordion">
              <v-expansion-panel
                v-for="(perguntas, criterio) in perguntasAgrupadas"
                :key="criterio"
              >
                ...
                <v-expansion-panel-text class="pa-6 bg-grey-lighten-5">
                  <div v-for="pergunta in perguntas" :key="pergunta.id_pergunta" class="mb-6">
                    ...
                    <v-radio-group v-model="respostas[pergunta.id_pergunta]" inline hide-details>
                      ...
                      <v-radio label="Fraco (25)" :value="25" color="orange-darken-2"></v-radio>
                      <v-radio label="Regular (50)" :value="50" color="orange"></v-radio>
                      <v-radio label="Bom (75)" :value="75" color="lime-darken-2"></v-radio>
            ...
            <h2 class="text-h6 font-weight-medium text-green-darken-4 mb-4">
              Considerações Finais
            </h2>
            <v-textarea
              v-model="observacoes"
              label="Observações textuais"
              rows="4"
              variant="outlined"
              class="mb-6"
            ></v-textarea>
        ...
        <v-card v-else variant="flat" border>
          <v-card-text class="text-center pa-10">
            ...
            <p class="text-body-1 mt-2 text-medium-emphasis">
              O seu voto de apoio para este projeto será contabilizado. Clique no botão abaixo para
              confirmar.
            </p>
          </v-card-text>
        </v-card>
      ...
      <v-dialog v-model="isConfirmDialogOpen" max-width="500px" persistent>
        <v-card>
          ...
          <v-card-text>
            Tem a certeza de que deseja submeter esta {{ isAluno ? 'votação' : 'avaliação' }}? Esta
            ação não pode ser desfeita.
          </v-card-text>
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="grey-darken-1" variant="text" @click="isConfirmDialogOpen = false"
              >Cancelar</v-btn
            >
            <v-btn color="green-darken-2" variant="flat" @click="confirmarSubmissao"
